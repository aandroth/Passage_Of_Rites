...
public class TESTING : MonoBehaviour
{
    ...
    public IEnumerator TestingSequence()
    {
        ...
        // Check AllData
        Assert.AreEqual(goPlayer.GetComponent<PlayerControls>().GetChangableData(), "Update,0,5,5,1,0");
        ...
        Assert.IsTrue(goPlayer.transform.position.y < yPositionPlayer);
        yield return new WaitForSeconds(1);
        Assert.AreEqual(goPlayer.GetComponent<PlayerControls>().GetChangableData(), $"Update,0,{goPlayer.transform.position.x},{goPlayer.transform.position.y},-1,0");
